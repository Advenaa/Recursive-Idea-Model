...
38. `RIM_ENABLE_DISAGREEMENT_ARBITRATION` default `1` in deep mode (resolve critique disagreements before synthesis)
39. `RIM_ARBITRATION_MAX_JOBS` default `2` (max disagreement arbitration calls per cycle)
40. `RIM_ENABLE_DEVILS_ADVOCATE_ARBITRATION` default `1` in deep mode (run follow-up devil's-advocate arbitration on low-confidence/escalated decisions)
41. `RIM_DEVILS_ADVOCATE_ROUNDS` default `1` (max devil's-advocate rounds per cycle)
42. `RIM_DEVILS_ADVOCATE_MIN_CONFIDENCE` default `0.72` (minimum arbitration confidence before triggering devil follow-up)
43. `RIM_ENABLE_EXECUTABLE_VERIFICATION` default `1` in deep mode (run safe executable checks from prefixed constraints)
44. `RIM_EXEC_VERIFY_MAX_CHECKS` default `5` (max executable constraint checks per cycle)
45. `RIM_ENABLE_PYTHON_EXEC_CHECKS` default `0` (allow `python_exec:` subprocess checks)
46. `RIM_PYTHON_EXEC_TIMEOUT_SEC` default `2` (timeout per `python_exec:` check)
