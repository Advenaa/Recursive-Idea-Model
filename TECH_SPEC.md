...
33. `RIM_ENABLE_VERIFICATION` default `1` in deep mode, `0` in fast mode (deterministic post-synthesis checks)
34. `RIM_VERIFY_MIN_CONSTRAINT_OVERLAP` default `0.6` (minimum lexical overlap for constraint coverage)
35. `RIM_VERIFY_MIN_FINDING_OVERLAP` default `0.35` (minimum lexical overlap for high-risk finding coverage)
36. `RIM_ENABLE_MEMORY_FOLDING` default `1` in deep mode (fold cycle context into compact tripartite memory)
37. `RIM_MEMORY_FOLD_MAX_ENTRIES` default `12` (max folded context entries carried to next cycle)
38. `RIM_MEMORY_FOLD_NOVELTY_FLOOR` default `0.35` (minimum fraction of new fold entries before degradation flag)
39. `RIM_MEMORY_FOLD_MAX_DUPLICATE_RATIO` default `0.5` (max duplicate ratio before degradation flag)
40. `RIM_ENABLE_DISAGREEMENT_ARBITRATION` default `1` in deep mode (resolve critique disagreements before synthesis)
41. `RIM_ARBITRATION_MAX_JOBS` default `2` (max disagreement arbitration calls per cycle)
42. `RIM_ENABLE_DEVILS_ADVOCATE_ARBITRATION` default `1` in deep mode (run follow-up devil's-advocate arbitration on low-confidence/escalated decisions)
43. `RIM_DEVILS_ADVOCATE_ROUNDS` default `1` (max devil's-advocate rounds per cycle)
44. `RIM_DEVILS_ADVOCATE_MIN_CONFIDENCE` default `0.72` (minimum arbitration confidence before triggering devil follow-up)
45. `RIM_ENABLE_SPECIALIST_ARBITRATION_LOOP` default `1` in deep mode (run specialist arbitration on diversity-flagged disagreements)
46. `RIM_SPECIALIST_ARBITRATION_MAX_JOBS` default `2` (max specialist arbitration reviews per cycle)
47. `RIM_SPECIALIST_ARBITRATION_MIN_CONFIDENCE` default `0.78` (minimum arbitration confidence before specialist follow-up)
48. `RIM_ENABLE_EXECUTABLE_VERIFICATION` default `1` in deep mode (run safe executable checks from prefixed constraints)
49. `RIM_EXEC_VERIFY_MAX_CHECKS` default `5` (max executable constraint checks per cycle)
50. `RIM_ENABLE_PYTHON_EXEC_CHECKS` default `0` (allow `python_exec:` subprocess checks)
51. `RIM_PYTHON_EXEC_TIMEOUT_SEC` default `2` (timeout per `python_exec:` check)
52. `RIM_ENABLE_ADVANCED_VERIFICATION` default `1` in deep mode (run solver/simulation/data-backed verification)
53. `RIM_ADV_VERIFY_MAX_CHECKS` default `4` (max advanced verification checks per cycle)
54. `RIM_ADV_VERIFY_SIMULATION_TRIALS` default `200` (Monte Carlo samples per simulation check)
55. `RIM_ADV_VERIFY_SIMULATION_MIN_PASS_RATE` default `0.7` (minimum simulation pass-rate threshold)
56. `RIM_ADV_VERIFY_SIMULATION_SEED` default `42` (deterministic simulation seed)
57. `RIM_ADV_VERIFY_DATA_PATH` default `rim/eval/data/benchmark_ideas.jsonl` (local reference dataset path for `data:` checks)
58. `RIM_ADV_VERIFY_EXTERNAL_TIMEOUT_SEC` default `8` (timeout for optional external advanced-verification adapters)
59. `RIM_ADV_VERIFY_EXTERNAL_SOLVER_CMD` optional command (external backend for `solver:` checks)
60. `RIM_ADV_VERIFY_EXTERNAL_SIMULATION_CMD` optional command (external backend for `simulate:` checks)
61. `RIM_ADV_VERIFY_EXTERNAL_DATA_CMD` optional command (external backend for `data:` checks)
62. Reference adapter template: `scripts/advanced_verify_adapter.py` (stdin JSON -> stdout JSON contract)
63. Optional adapter env: `RIM_ADV_VERIFY_ADAPTER_SOLVER_BACKEND=z3` (use z3 backend when available, fallback otherwise)
