...
@dataclass
class EngineAgents:
    decompose: Callable[..., Awaitable[tuple[list[DecompositionNode], str, dict[str, Any]]]] = decompose_idea
    run_critics: Callable[..., Awaitable[list[CriticFinding]]] = run_critics
    reconcile_findings: Callable[..., dict[str, Any]] = reconcile_findings
    run_arbitration: Callable[..., Awaitable[tuple[list[dict[str, Any]], list[str]]]] = run_arbitration
    synthesize_idea: Callable[..., Awaitable[tuple[dict[str, Any], list[str]]]] = synthesize_idea
    verify_synthesis: Callable[..., dict[str, Any]] = verify_synthesis
    run_executable_verification: Callable[..., dict[str, Any]] = run_executable_verification
    run_advanced_verification: Callable[..., dict[str, Any]] = run_advanced_verification
    decide_next_cycle: Callable[..., Any] = decide_next_cycle
    severity_counts: Callable[[list[CriticFinding]], tuple[int, int]] = severity_counts
    fold_cycle_memory: Callable[..., dict[str, Any]] = fold_cycle_memory
    fold_to_memory_entries: Callable[..., list[dict]] = fold_to_memory_entries
    build_spawn_plan: Callable[..., dict[str, Any]] = build_spawn_plan


class RimExecutionEngine:
    def __init__(
        self,
        ...
        self.router = router
        self.agents = agents or EngineAgents()
    ...
    async def execute_run(self, run_id: str, request: AnalyzeRequest) -> AnalyzeResult:
        ...
        try:
            ...
            for cycle in range(1, max_cycles + 1):
                ...
                nodes, decompose_provider, decompose_meta = await self.agents.decompose(
                    provider_session,
                    current_idea,
                    settings,
                    domain=request.domain,
                ...
                findings = await self.agents.run_critics(
                    provider_session,
                    nodes,
                    settings,
                    domain=request.domain,
                ...
                reconciliation = self.agents.reconcile_findings(
                    findings,
                ...
                try:
                    synthesis, synthesis_providers = await self.agents.synthesize_idea(
                        provider_session,
                        current_idea,
                        nodes,
                        findings,
                ...
                if verification_enabled:
                    verification = self.agents.verify_synthesis(
                        synthesis=synthesis,
                        findings=findings,
                        constraints=request.constraints,
                ...
                if executable_verification_enabled:
                    exec_verification = self.agents.run_executable_verification(
                        constraints=request.constraints,
                        synthesis=synthesis,
                        findings=findings,
                ...
                if advanced_verification_enabled:
                    advanced_verification = self.agents.run_advanced_verification(
                        constraints=request.constraints,
                        synthesis=synthesis,
                        findings=findings,
                ...
                high_findings, critical_findings = self.agents.severity_counts(findings)
                decision = self.agents.decide_next_cycle(
                    cycle=cycle,
                    max_cycles=max_cycles,
                ...
                if enable_memory_folding:
                    fold_payload = self.agents.fold_cycle_memory(
                        cycle=cycle,
                    ...
                    cycle_fold_entries = self.agents.fold_to_memory_entries(
                        fold_payload,
                        domain=request.domain,
                    )
